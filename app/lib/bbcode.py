...
def youku_url(video_id, with_html5=False):

    if with_html5:
        video_url = '<br/><video width="120" height="100"src="http://v.youku.com/player/getRealM3U8/vid/'+video_id+'/type/video.m3u8" controls>'
        video_url += '<embed src="http://player.youku.com/player.php/sid/' + video_id + '/v.swf" allowFullScreen="true" quality="high" width="480" height="400" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed><br/><br/>'
        video_url += '</video>'
        return video_url
    else:
        video_url = '<br/><embed src="http://player.youku.com/player.php/sid/' + video_id + '/v.swf" allowFullScreen="true" quality="high" width="480" height="400" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed><br/><br/>'
        return video_url  
...
class LinkTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        ...
        if not self.url:
            return u""
# <video width="480" height="320" src="http://v.youku.com/player/getRealM3U8/vid/XMzkxMTM3MDQw/type//video.m3u8" controls>
#     <embed src="http://player.youku.com/player.php/sid/XMzkxMTM3MDQw/v.swf" allowFullScreen="true" quality="high" width="480" height="320" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed>
# </video>
        #return parser.useragent

        if self.domain:
            video_url = u''
            # if self.domain == 'v.youku.com':
            #     url = PostMarkup.standard_replace_no_break(url)
            #     video_id = re.findall('id_([a-zA-Z0-9\=]+)', url)
            #     if len(video_id) > 0:
            #         if parser.useragent == 'iOS' or parser.useragent == 'android':
            #             return youku_url(video_id[0], with_html5=True)
            #             #video_url = '<video width="100%" height="200"src="http://v.youku.com/player/getRealM3U8/vid/'+video_id[0]+'/type/video.m3u8" controls>'
            #             #video_url += '<embed src="http://player.youku.com/player.php/sid/' + video_id[0] + '/v.swf" allowFullScreen="true" quality="high" width="480" height="400" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed><br/><br/>'
            #             #video_url += '</video>'
            #         else:
            #             return youku_url(video_id[0])
            #             #video_url += '<embed src="http://player.youku.com/player.php/sid/' + video_id[0] + '/v.swf" allowFullScreen="true" quality="high" width="480" height="400" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed><br/><br/>'
                    

            return video_url+u'<a href="%s" target="_blank">' % PostMarkup.standard_replace_no_break(self.url)
...
class VideoTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        ...
        video_url = PostMarkup.standard_replace_no_break(url)
        video_id = re.findall('id_([a-zA-Z0-9\=]+)', video_url)
        if len(video_id) > 0:
            if parser.useragent == 'iOS' or parser.useragent == 'android':
                return youku_url(video_id[0], with_html5=True)
            else:
...
class AppTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        ...
        video_url = PostMarkup.standard_replace_no_break(url)
        video_id = re.findall('id([0-9]+)', video_url)
        if len(video_id) > 0:
            if parser.useragent == 'iOS' or parser.useragent == 'android':
                return youku_url(video_id[0], with_html5=True)
            else:
...
class ImgTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        ...
        return u'<br/><img src="%s"></img>' % PostMarkup.standard_replace_no_break(url)
...
class BrTag(TagBase):
    ...
    def __init__(self, name, **kwargs):
        super(BrTag, self).__init__( name, inline=True)
        
    def open(self, parser, params, *args):
        if params.strip():
            self.auto_close = True            
        super(BrTag, self).open(parser, params, *args)
...
def _tests():
    ...
    tests.append(u"[b]Hello Andre[/b]")
    tests.append(u"[google]Andre[/google]")
